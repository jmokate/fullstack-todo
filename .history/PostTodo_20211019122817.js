...
const postTodo = async (todo) => {
  let pool = await pgAccess.connectToDb();
    try {
      await pool.query("BEGIN");
      let result = await pool.query("INSERT INTO todos(text) VALUES($1)", [todo]);
      await pool.query("COMMIT");
...
const putTodo = async (id) => {
  ...
  try {
    await pool.query("BEGIN");
    await pool.query("UPDATE todos SET is_checked = NOT is_checked WHERE id = ($1) RETURNING id", [id]);
    await pool.query("COMMIT");
  } catch (err) {
