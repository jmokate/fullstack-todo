...
function Todo({setIsChecked, handleDelete, item, isChecked}) {
  ...
  const strikeThrough = () => {
    item.checked ? setListItemTextDecoration("line-through") : setListItemTextDecoration("none")
  }
  ...
  return(
    <>
      <Row>
        <Col>
          <input type="checkbox" onClick={returnIsCheckedValue} onChange={() => handleCheck(item.id)} />
        </Col>
        <Col >
